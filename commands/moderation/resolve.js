...
class Revoke {
    constructor() {
        this.help = {
            name: 'resolve',
            usage: 'resolve <user_resolvable> <warn_id> -r <reason> -s <screenshot_url_or_attachment>',
            description: "Resolve a warn or all warns from a member of the server, reason is optional and can be added after. Screenshot is optional as well, it may be followed by the (single) url of the screenshot or stay blank if the screenshot is attached",
            detailedUsage: "The case ID is shown with the `moderation` command\n`{prefix}revoke @Baguette 1` Will resolve the warn #1 of the user `Baguette`\nDon't specify any case ID to revoke all warns"
        }
        this.conf = {
            guildOnly: true
        }
            ...
            try {
                ...
                if (!warnID) {
                    let confirmation = await message.awaitReply({
                        message: {
                            embed: {
                                description: `This will resolve all warns of the user \`${member.tag}\`, are you sure you want to do that?\nReply with \`yes\` to confirm or anything else to abort`
                            }
                        }
                    });
                    ...
                    await ModerationHandler.registerCase(client, {
                        ...
                        performedAction: 'Resolved all warns'
                    });
                    return resolve(await message.channel.createMessage(`:white_check_mark: Alright, I resolved all warns from \`${member.tag}\``));
                }
                ...
                await ModerationHandler.registerCase(client, {
                    ...
                    guild: message.guild,
                    screenshot: screenshot,
                    performedAction: `Resolved warn #${Math.round(warnID)}`
                });
                resolve(await message.channel.createMessage(`:white_check_mark: Successfully resolved the warn \`#${Math.round(warnID)}\` of the user \`${member.tag}\``));
            } catch (err) {
                reject(err);
            }
        });
